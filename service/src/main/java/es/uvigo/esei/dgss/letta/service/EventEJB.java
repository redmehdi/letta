...
public class EventEJB {
     ...
     *
     * @return A sorted {@link List} containing all the highlighted {@link
     *         Event Events}, magically sorted and counting from the received
     *         start point.
     */
    @PermitAll
    public List<Event> listHighlighted() {
        ...
        ).setMaxResults(numHighlightedEvents).getResultList();
    }
     ...
     *
     * @param event Indicates the {@link Event} that the {@link User} want to
     *        register.
     *
     ...
     */
    @RolesAllowed("USER")
    public final void registerToEvent(
        final Event event
    ) throws EventAlredyJoinedException, SecurityException {
        final User user = auth.getCurrentUser();

        final List<Event> eventsOfUser = user.getUsersJoinsEvents();
        final List<User>  usersOfEvent = event.getEventsJoinedByUsers();
