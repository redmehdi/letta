...
public final class EventsDataset {
	...
	public static Event[] events() {
		...
		return new Event[] {
				new Event(1, LITERATURE, "Example1 literature",
				...
				new Event(7, TV, "Example1 tv", "This is a description tv 1",
						new Date(946684861000L), "Location X", users[1]),
				new Event(8, TV, "Example2 tv", "This is a description tv 2",
						new Date(946684861000L), "Location X", users[1]),
				new Event(9, SPORTS, "Example1 sports",
						"This is a description sports 1",
						new Date(946684861000L), "Location X", users[1]),
				new Event(10, SPORTS, "Example2 sports",
						"This is a description sports 2",
						new Date(946684861000L), "Location X", users[1]),
				new Event(11, INTERNET, "Example1 internet",
				...
				new Event(12, INTERNET, "Example2 internet",
						...
						new Date(946684861000L), "Location X", users[2]),
				new Event(13, TRAVELS, "Example1 travels",
						"This is a description travels 1",
						new Date(946684861000L), "Location X", users[2]),
				new Event(14, TRAVELS, "Example2 travels",
						"This is a description travels 2",
						new Date(946684861000L), "Location X", users[2]),
				new Event(15, THEATRE, "Example1 theatre",
				...
				new Event(20, CINEMA, "Example3 cinema",
						...
						new Date(946684861000L), "Location X", users[3]),
				new Event(21, TV, "Example3 tv", "This is a description tv 3",
	...
	public static List<Event> eventsAsList() {
	...
	/**
	 * An array of {@link Event} that have in them title or description a
	 * pattern provided
	 *
	 * @param pattern
	 ...
	 *         description
	 */
	public static Event[] eventsWithTitleOrDescriptionContaining(
			final String pattern) {
		return filterEvents(e -> e.getTitle().contains(pattern)
				|| e.getShortDescription().contains(pattern));
	}
	 ...
	 *
	 * @param <A>
	 *            The type of the resulting mapping function.
	 * @param f
	 *            The mapper {@link Function} from {@link Event} to any type.
	 *
	 ...
	 *
	 * @param cmp
	 *            The {@link Comparator} to sort the {@link Event Events}.
	 *
	 ...
	 *
	 * @param f
	 *            The function mapping an {@link Event} to a {@link Comparable}
	 *            instance.
	 * @param <C>
	 *            The type of the {@link Comparable} by which an {@link Event}
	 *            should be compared.
	 *
	...
	public static Event newEvent() {
		return new Event(TRAVELS, "New travels event",
				"New travels description", new Date(587684861000L),
	...
	public static Event newEventWithoutCreator() {
		return new Event(LITERATURE, "New literature event",
				"This is a description", new Date(946684860000L), "Location X");
	}
	...
	public static Event[] eventsWithTwoJoinedUsers() {
		...
		return stream(events()).map(event -> {
			...
			final User[] joins = groups[index];

			event.getEventsJoinedByUsers().addAll(asList(joins));
	...
	public static Event[] filterEventsWithTwoJoinedUsers(
		...
		return stream(eventsWithTwoJoinedUsers()).filter(f)
				.toArray(Event[]::new);
	}

	/**
	 * Returns all the {@link Event} created by an {@link User}
	 * 
	 * @param login
	 *            indicates the {@link User} login
	 * @return all the {@link Event} created
	 */
	public static List<Event> eventsWithCreator(String login) {
		List<Event> eventsCreated = new LinkedList<Event>();
		for (Event e : events()) {
	...
	/**
	 * Returns all the {@link Event} joined by an {@link User}
	 * 
	 * @param login
	 *            indicates the {@link User} login
	 * @return all the {@link Event} joined
	 */
	public static List<Event> eventsWithUserJoined(String login) {
		List<Event> eventsJoined = new LinkedList<Event>();
		for (Event e : eventsWithTwoJoinedUsers()) {
			if (e.getCreator().equals(login)) {
				eventsJoined.add(e);
			}
		}
